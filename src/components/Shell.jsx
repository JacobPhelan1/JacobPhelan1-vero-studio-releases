...
export default function Shell({ production, onProductionChange, onClose, controller }) {
  ...
  return <div className="app-shell">
    ...
    <main className="main">
      <header className="topbar"><div><p>{page.toUpperCase()}</p><h1>{production.productionName}</h1></div><div className="health"><span className={controller.bridge.status === "Connected" ? "ok" : "bad"}>BRIDGE {controller.bridge.status.toUpperCase()}</span><span className={controller.vmix.status === "Connected" ? "ok" : "bad"}>VMIX {controller.vmix.status.toUpperCase()}</span></div></header>
