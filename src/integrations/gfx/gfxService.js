...
async function parseResponse(response) {
  ...
  if (!response.ok || result.ok === false) throw new GfxProtocolError(result.error || result.message || `VERO GFX request failed (HTTP ${response.status}).`, { status: response.status, code: response.status === 409 ? "VERSION_INCOMPATIBLE" : "GFX_REQUEST_FAILED", protocolVersion: result.protocolVersion });
  return result;
}

export class GfxService {
  constructor({ baseUrl = APP_CONFIG.services.gfxProtocolUrl, fetcher = null } = {}) { this.baseUrl = baseUrl.replace(/\/$/, ""); this.fetcher = fetcher || ((...args) => globalThis.fetch(...args)); this.instanceId = `vero-studio-${crypto.randomUUID()}`; this.connected = false; this.cursor = 0; this.application = null; }
